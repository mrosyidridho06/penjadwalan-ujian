...
<div class="section">
    <div class="section-header">
        <h1>Edit Internal Kemajuan Penelitian</h1>
    </div>
    <div class="card">
        <div class="card-body table-responsive">
        <form action="{{ route('kemajuan-penelitian.update', $kemajuanPenelitian->id) }}" method="POST" enctype="multipart/form-data">
            @csrf
            @method('PUT')
            <div class="form-group">
                <label for="jurusan">Nama</label>
                <input type="text" class="form-control" name="nama" value="{{ $kemajuanPenelitian->mahasiswa->user->name }}" readonly>
                <br>
                <label for="judul">Judul</label>
                <textarea name="judul" class="form-control">{{ $kemajuanPenelitian->judul }}</textarea>
                <br>
                <label for="ruangan_id">Ruangan</label>
                <select name="ruangan_id" class="form-control">
                    @foreach ($ruangan as $itemruang)
                        @if ($kemajuanPenelitian->ruangan_id == $itemruang->id)
                ...
                <input type="date" class="form-control" name="tanggal" value="{{ $kemajuanPenelitian->tanggal }}">
                <br>
                <label for="sesi_id">Sesi</label>
                <select name="sesi_id" class="form-control">
                    @foreach ($sesi as $itemsesi)
                        @if ($kemajuanPenelitian->sesi_id == $itemsesi->id)
                ...
                <div class="form-control mb-2">
                    <a href="{{ asset("/skripsi2/kemajuan_penelitian/" . $kemajuanPenelitian->draft) }}" target="_blank">{{ $kemajuanPenelitian->draft }}</a>
                </div>
