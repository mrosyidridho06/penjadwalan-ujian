...
<div class="section">
    <div class="section-header">
        <h1>Edit Internal Tinjauan Pustaka</h1>
    </div>
    <div class="card">
        <div class="card-body table-responsive">
        <form action="{{ route('tinjauan-pustaka.update', $tinjauanPustaka->id) }}" method="POST" enctype="multipart/form-data">
            @csrf
            @method('PUT')
            <div class="form-group">
                <label for="jurusan">Nama</label>
                <input type="text" class="form-control" name="nama" value="{{ $tinjauanPustaka->mahasiswa->user->name }}" readonly>
                <br>
                <label for="judul">Judul</label>
                <textarea name="judul" class="form-control">{{ $tinjauanPustaka->judul }}</textarea>
                <br>
                <label for="ruangan_id">Ruangan</label>
                <select name="ruangan_id" class="form-control">
                    @foreach ($ruangan as $itemruang)
                        @if ($tinjauanPustaka->ruangan_id == $itemruang->id)
                ...
                <input type="date" class="form-control" name="tanggal" value="{{ $tinjauanPustaka->tanggal }}">
                <br>
                <label for="sesi_id">Sesi</label>
                <select name="sesi_id" class="form-control">
                    @foreach ($sesi as $itemsesi)
                        @if ($tinjauanPustaka->sesi_id == $itemsesi->id)
                ...
                <div class="form-control mb-2">
                    <a href="{{ asset("/skripsi1/tinjauan_pustaka/" . $tinjauanPustaka->draft) }}" target="_blank">{{ $tinjauanPustaka->draft }}</a>
                </div>
        ...
        <a href="{{ route('tinjauan-pustaka.index') }}" class="btn btn-secondary">Kembali</a>
    </div>
    </div>
</div>
@endsection
