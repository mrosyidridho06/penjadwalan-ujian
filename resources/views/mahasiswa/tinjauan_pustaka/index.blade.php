@extends('layouts.app')
@section('title', 'Ujian Tinjauan Pustaka')
@section('content')
<div class="section">
    <div class="section-header">
        <h1>Internal Tinjauan Pustaka</h1>
    </div>
    <div class="card">
        @if(auth()->check())
            @if (auth()->user()->role == 'mahasiswa')
                @if ($tipus->where('mahasiswa_id','=', auth()->user()->mahasiswa->id)->count() == 0)
                <div class="card-header d-flex justify-content-end">
                    <a href="{{ route('tinjauan-pustaka.create') }}" type="button" class="btn btn-light">
            ...
            <table class="table table-hover" id="myTable">
                ...
                <tbody>
                    ...
                    <tr>
                        ...
                        <td><a href="{{ asset("/skripsi1/tinjauan_pustaka/" . $item->draft) }}" target="_blank"> {{ $item->draft }}</a></td>
                        <td>
                            <div class="dropdown">
                                ...
                                <div class="dropdown-menu dropdown-primary">
                                    <a class="dropdown-item" href="{{route('tinjauan-pustaka.edit',$item->id)}}"><i class="fa fa-edit"></i> Edit</a>
                                    <form action="{{route('tinjauan-pustaka.destroy', $item->id)}}" method="POST">
                                        @csrf
                                        @method('DELETE')
                        ...
                        {{-- <td>
                            <form action="{{ route('ba.tipus') }}" method="POST">
                                @csrf
