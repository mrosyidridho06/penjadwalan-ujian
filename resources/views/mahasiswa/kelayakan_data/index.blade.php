@extends('layouts.app')
@section('title', 'Sidang Kelayakan Data')
@section('content')
<div class="section">
    <div class="section-header">
        <h1>Internal Kelayakan Data</h1>
    </div>
    <div class="card">
        @if(auth()->check())
            @if (auth()->user()->role == 'mahasiswa')
                @if ($kelayakan->where('mahasiswa_id','=', auth()->user()->mahasiswa->id)->count() == 0)
                <div class="card-header d-flex justify-content-end">
                    <a href="{{ route('kelayakan-data.create') }}" type="button" class="btn btn-light">
            ...
            <table class="table table-hover" id="myTable">
                ...
                <tbody>
                    ...
                    <tr>
                        ...
                        <td><a href="{{ asset("/skripsi2/kelayakan_data/" . $item->draft) }}" target="_blank"> {{ $item->draft }}</a></td>
                        <td>
                            <div class="dropdown">
                                ...
                                <div class="dropdown-menu dropdown-primary">
                                    <a class="dropdown-item" href="{{route('kelayakan-data.edit',$item->id)}}"><i class="fa fa-edit"></i> Edit</a>
                                    <form action="{{route('kelayakan-data.destroy', $item->id)}}" method="POST">
                                        @csrf
                                        @method('DELETE')
