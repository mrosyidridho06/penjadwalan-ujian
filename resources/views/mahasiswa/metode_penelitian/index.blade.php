@extends('layouts.app')
@section('title', 'Ujian Metode Penelitian')
@section('content')
<div class="section">
    <div class="section-header">
        <h1>Internal Metode Penelitian</h1>
    </div>
    <div class="card">
        @if(auth()->check())
            @if (auth()->user()->role == 'mahasiswa')
                @if ($metpen->where('mahasiswa_id','=', auth()->user()->mahasiswa->id)->count() == 0)
                <div class="card-header d-flex justify-content-end">
                    <a href="{{ route('metode-penelitian.create') }}" type="button" class="btn btn-light">
            ...
            <table class="table table-hover" id="myTable">
                ...
                <tbody>
                    ...
                    <tr>
                        ...
                        <td><a href="{{ asset("/skripsi1/metode_penelitian/" . $item->draft) }}" target="_blank"> {{ $item->draft }}</a></td>
                        <td>
                            <div class="dropdown">
                                ...
                                <div class="dropdown-menu dropdown-primary">
                                    <a class="dropdown-item" href="{{route('metode-penelitian.edit',$item->id)}}"><i class="fa fa-edit"></i> Edit</a>
                                    <form action="{{route('metode-penelitian.destroy', $item->id)}}" method="POST">
                                        @csrf
                                        @method('DELETE')
                        ...
                        {{-- <td>
                            <form action="{{ route('ba.metpen') }}" method="POST">
                                @csrf
