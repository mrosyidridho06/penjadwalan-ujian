@extends('layouts.app')
@section('title', 'Sidang Internal Prosedural')
@section('content')
<div class="section">
    <div class="section-header">
        <h1>Internal Prosedural</h1>
    </div>
    <div class="card">
        @if(auth()->check())
            @if (auth()->user()->role == 'mahasiswa')
                @if ($prosedural->where('mahasiswa_id','=', auth()->user()->mahasiswa->id)->count() == 0)
                <div class="card-header d-flex justify-content-end">
                    <a href="{{ route('internal-prosedural.create') }}" type="button" class="btn btn-light">
            ...
            <table class="table table-hover" id="myTable">
                ...
                <tbody>
                    ...
                    <tr>
                        ...
                        <td><a href="{{ asset("/skripsi2/internal_prosedural/" . $item->draft) }}" target="_blank"> {{ $item->draft }}</a></td>
                        <td>
                            <div class="dropdown">
                                ...
                                <div class="dropdown-menu dropdown-primary">
                                    <a class="dropdown-item" href="{{route('internal-prosedural.edit',$item->id)}}"><i class="fa fa-edit"></i> Edit</a>
                                    <form action="{{route('internal-prosedural.destroy', $item->id)}}" method="POST">
                                        @csrf
                                        @method('DELETE')
