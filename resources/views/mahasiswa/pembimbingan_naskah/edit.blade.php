...
<div class="section">
    ...
    <div class="card">
        ...
        <form action="{{ route('pembimbingan-naskah.update', $naskahProposal->id) }}" method="POST" enctype="multipart/form-data">
            @csrf
            @method('PUT')
            <div class="form-group">
                <label for="jurusan">Nama</label>
                <input type="text" class="form-control" name="nama" value="{{ $naskahProposal->mahasiswa->user->name }}" readonly>
                <br>
                <label for="judul">Judul</label>
                <textarea name="judul" class="form-control">{{ $naskahProposal->judul }}</textarea>
                <br>
                <label for="ruangan_id">Ruangan</label>
                <select name="ruangan_id" class="form-control">
                    @foreach ($ruangan as $itemruang)
                        @if ($naskahProposal->ruangan_id == $itemruang->id)
                ...
                <input type="date" class="form-control" name="tanggal" value="{{ $naskahProposal->tanggal }}">
                <br>
                <label for="sesi_id">Sesi</label>
                <select name="sesi_id" class="form-control">
                    @foreach ($sesi as $itemsesi)
                        @if ($naskahProposal->sesi_id == $itemsesi->id)
                ...
                <div class="form-control mb-2">
                    <a href="{{ asset("/skripsi1/pembimbingan_naskah/" . $naskahProposal->draft) }}" target="_blank">{{ $naskahProposal->draft }}</a>
                </div>
        ...
        <a href="{{ route('pembimbingan-naskah.index') }}" class="btn btn-secondary">Kembali</a>
    </div>
    </div>
</div>
@endsection
