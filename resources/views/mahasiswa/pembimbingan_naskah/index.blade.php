@extends('layouts.app')
@section('title', 'Sidang Pembimbingan Naskah')
@section('content')
<div class="section">
    ...
    <div class="card">
        @if(auth()->check())
            @if (auth()->user()->role == 'mahasiswa')
                @if ($pn->where('mahasiswa_id','=', auth()->user()->mahasiswa->id)->count() == 0)
                <div class="card-header d-flex justify-content-end">
                    <a href="{{ route('pembimbingan-naskah.create') }}" type="button" class="btn btn-light">
            ...
            <table class="table table-hover" id="myTable">
                ...
                <tbody>
                    ...
                    <tr>
                        ...
                        <td><a href="{{ asset("/skripsi1/pembimbingan_naskah/" . $item->draft) }}" target="_blank"> {{ $item->draft }}</a></td>
                        <td>
                            <div class="dropdown">
                                ...
                                <div class="dropdown-menu dropdown-primary">
                                    <a class="dropdown-item" href="{{route('pembimbingan-naskah.edit',$item->id)}}"><i class="fa fa-edit"></i> Edit</a>
                                    <form action="{{route('pembimbingan-naskah.destroy', $item->id)}}" method="POST">
                                        @csrf
                                        @method('DELETE')
                        ...
                        {{-- <td>
                            <form action="{{ route('ba.pn') }}" method="POST">
                                @csrf
